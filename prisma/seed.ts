...
async function seedReferences() {
  ...
  const attributes = [
    ...
    },
    {
      ...
      dataType: "NUMBER",
      iconUrl: "expand-outline",
    },
    {
      slug: "floor",
      label: "Number of Floors",
      dataType: "NUMBER",
      iconUrl: "layers-outline",
    },
    {
      slug: "furnishing",
      label: "Furnishing",
      dataType: "STRING",
      iconUrl: "sofa-outline",
    },
    {
      slug: "garage",
...
async function seedPermissions() {
  ...
  const permissions = [
    ...
    { action: "dispute.resolve", description: "Resolve disputes" },
    // Finance
    {
...
async function seedRolesAndUsers() {
  ...
  await prisma.userRole.upsert({
    where: {
      userId_roleId: { userId: landlord.id, roleId: roleMap["LANDLORD"] },
    },
    update: {},
...
async function seedProperties() {
  ...
  const attrBed = await prisma.propertyAttributeType.findUniqueOrThrow({
    where: { slug: "bedroom" },
  });
  const attrWifi = await prisma.propertyAttributeType.findUniqueOrThrow({
    where: { slug: "wifi_speed" },
  });

  // 1. Luxury Villa (Verified)
  // We don't upsert properties typically, but we check existence to avoid dups in seed
  const villaExists = await prisma.property.findFirst({
  ...
  if (!villaExists) {
    await prisma.property.create({
      data: {
        ...
        description:
          "Experience luxury living in this 3-bedroom villa with a private infinity pool.",
  ...
  const kostExists = await prisma.property.findFirst({
    where: { title: "Cozy Student Room near UI" },
  });
  if (!kostExists) {
    await prisma.property.create({
      data: {
        ...
        description:
          "Affordable room for students, walking distance to campus.",
  ...
  const events = [
    {
      ...
      role: "TENANT",
      baseImpact: -5.0,
      description: "Paid rent after due date",
    },
    {
    ...
    },
    {
      ...
      role: "LANDLORD",
      baseImpact: 3.0,
      description: "Responded within 30 mins",
    },
    {
      code: "FAKE_LISTING",
      category: "ACCURACY",
      role: "LANDLORD",
      baseImpact: -50.0,
      description: "Posted a fake listing",
    },
    {
      code: "KYC_VERIFIED",
      category: "COMPLIANCE",
    ...
    },
    {
      ...
      role: "TENANT",
      baseImpact: -15.0,
      description: "Found at fault in dispute",
    },
  ];
...
async function seedBookingsAndDisputes() {
  ...
  const villa = await prisma.property.findFirstOrThrow({
    where: { title: "Sunset Paradise Villa" },
  });
  const monthly = await prisma.billingPeriod.findUniqueOrThrow({
    where: { slug: "monthly" },
    ...
    data: {
      ...
      disputes: {
        create: {
          ...
          description:
            "The air conditioner in the master bedroom has been broken for 3 days and the landlord is not responding.",
          status: "OPEN",
        },
      },
    },
  ...
  // 3. Create an External Block (iCal)
  // This simulates a booking from Airbnb blocking next week
  ...
  await prisma.booking.create({
    data: {
      propertyId: villa.id,
      tenantId: landlord.id, // Assigned to landlord for blocks
      startDate: nextWeekStart,
      endDate: nextWeekEnd,
      status: "BLOCKED",
      source: "EXTERNAL_ICAL",
      externalId: "airbnb-import-uid-12345",
    },
  });
}

async function seedReviews() {
  console.log("⭐ [Seed] Reviews & Ratings...");
  ...
  // 1. Create a COMPLETED booking in the past to allow review
  ...
  const pastBooking = await prisma.booking.create({
    data: {
      ...
      billingPeriodId: monthly.id,
      startDate: pastDate,
      endDate: new Date(new Date(pastDate).setMonth(pastDate.getMonth() + 1)),
      status: "COMPLETED",
      invoices: {
        create: {
          amount: 25000000,
  ...
  await prisma.review.create({
    data: {
      ...
      rating: 5,
      comment:
        "Absolutely amazing villa! The pool is stunning and the host is very responsive. Highly recommended.",
    },
  });

  // 3. Update Property Stats (Since subscribers don't run during seed)
    ...
    data: {
      ...
      messages: {
        create: [
          {
            senderId: tenant.id,
            content: "Hi, is this villa still available?",
          },
          {
            senderId: landlord.id,
            content: "Yes, it is available for next month.",
          },
        ],
      },
    },
